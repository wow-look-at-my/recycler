...
// LockPath returns the file whose lock names the running daemon. One daemon
// holds it per user, which is what keeps a recycle every few seconds from
// starting a daemon every few seconds.
...
// Ensure starts the disk-pressure daemon if one is not already running, by
// running exe with the "daemon" argument detached from this process. It reports
// whether it started one.
//
// Spawning is the caller's decision rather than something a recycle does: this
// module is a library, and a library that forks a process out of an ordinary
