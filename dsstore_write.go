...
// buildDSStore writes a complete .DS_Store file holding exactly these records.
// The whole file is rebuilt rather than edited in place, which is what keeps
// the buddy allocator's bookkeeping honest.
func buildDSStore(records []dsRecord) ([]byte, error) {
	sorted := make([]dsRecord, len(records))
	copy(sorted, records)
...
func dsBuildLevels(records []dsRecord) ([][][]dsRecord, error) {
	if len(records) == 0 {
		return [][][]dsRecord{{{}}}, nil
	}
	current := records
...
func dsEncodeNodes(levels [][][]dsRecord) [][]byte {
	var (
		encoded  [][]byte
...
func appendDSRecord(buf []byte, r dsRecord) []byte {
