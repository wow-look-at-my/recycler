...
// The disk-pressure daemon. Recycling defers a deletion rather than performing
// one, which only works while there is somewhere to defer it to: a bin nobody
// empties fills the filesystem, and the recycled copy is what is holding the
// space. The daemon is what keeps that promise affordable, by giving back the
// oldest items when the filesystem gets tight.
//
// This is the one thing in the package that destroys anything, and it is
...
func Sweep() ([]Eviction, error) {
	...
	return sweepItems(b, items, diskFree)
}
...
func sweepItems(b backend, items []Item, free func(string) (uint64, uint64, error)) ([]Eviction, error) {
	...
	for _, group := range groupByFilesystem(items) {
		...
		for _, it := range group.items {
			...
			if it.Size == SizeUnknown || it.Size < 0 {
				continue
			}
			ev := Eviction{Item: it}
...
func groupByFilesystem(items []Item) []filesystemGroup {
	order := make([]string, 0, 4)
	byDir := make(map[string][]Item, 4)
...
// too.
// It returns [ErrDaemonRunning] when another daemon already holds the lock, so
// a second one started by hand stands down rather than sweeping in parallel.
