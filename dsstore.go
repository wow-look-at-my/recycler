...
// A dsRecord is one property of one file in the directory the .DS_Store
...
// dsUstr builds a record holding a Unicode string, the type both put back
// records use.
func dsUstr(name, code, value string) dsRecord {
	...
	return dsRecord{Name: name, Code: code, Type: "ustr", Data: data}
}

// ustr decodes a "ustr" record's value. It reports false for a record of any
// other type.
func (r dsRecord) ustr() (string, bool) {
...
// parseDSStore decodes every record in a .DS_Store file, in the order they are
// stored. A file with no B-tree yields no records and no error.
func parseDSStore(data []byte) ([]dsRecord, error) {
...
func readDSRecord(r *dsReader) (dsRecord, error) {
	length, err := r.u32()
	if err != nil {
		return dsRecord{}, err
	}
	if int(length) > (len(r.buf)-r.pos)/2 {
		return dsRecord{}, fmt.Errorf("%w: name of %d characters runs past the block", errBadDSStore, length)
	}
