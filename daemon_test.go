...
func (f *fakeBackend) list() ([]Item, error)                  { return nil, nil }
...
func item(name string, size int64, ageHours int) Item {
	return Item{
...
func TestASweepLeavesAFilesystemWithRoomAlone(t *testing.T) {
	b := &fakeBackend{}
	items := []Item{item("old.txt", 500, 48), item("new.txt", 500, 1)}
...
func TestAnItemOfUnknownSizeIsNeverEvicted(t *testing.T) {
	b := &fakeBackend{}
	items := []Item{
		item("mystery.txt", SizeUnknown, 96),
...
func TestASecondDaemonStandsDown(t *testing.T) {
	...
	unlock, held, err := tryLockDaemon(lock)
	require.NoError(t, err)
	require.True(t, held)
	defer unlock()

	_, held2, err := tryLockDaemon(lock)
	require.NoError(t, err)
	assert.False(t, held2, "two daemons took the same lock")

	assert.ErrorIs(t, RunDaemon(t.Context(), time.Second, nil), ErrDaemonRunning)
}

// diskFree is what the daemon polls, so it has to answer for a real path.
func TestDiskFreeReportsARealFilesystem(t *testing.T) {
	avail, total, err := diskFree(t.TempDir())
	require.NoError(t, err)
	assert.Positive(t, total)
	assert.LessOrEqual(t, avail, total)
}

// EnsureDaemon must refuse a test binary. os.Executable() under `go test` is
// the test binary, and running one with a "daemon" argument re-runs the whole
// suite, which recycles, which lands back here: a fork bomb.
