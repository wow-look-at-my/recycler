...
// binMetadata is the content of a $I file.
...
// parseBinMetadata decodes a $I file.
func parseBinMetadata(data []byte) (binMetadata, error) {
	if len(data) < binMetaHeaderSize {
		return binMetadata{}, fmt.Errorf("%w: %d bytes is too short", errBadMetadata, len(data))
	}
	...
	case 1:
		if len(data) < binMetaHeaderSize+binMetaV1PathLen*2 {
			return binMetadata{}, fmt.Errorf("%w: truncated version 1 path", errBadMetadata)
		}
		chars = decodeUTF16LE(data[binMetaHeaderSize : binMetaHeaderSize+binMetaV1PathLen*2])
	case 2:
		if len(data) < binMetaHeaderSize+4 {
			return binMetadata{}, fmt.Errorf("%w: truncated version 2 header", errBadMetadata)
		}
		...
		if count < 0 || start+count*2 > len(data) {
			return binMetadata{}, fmt.Errorf("%w: path length %d exceeds the file", errBadMetadata, count)
		}
		chars = decodeUTF16LE(data[start : start+count*2])
	default:
		return binMetadata{}, fmt.Errorf("%w: unknown version %d", errBadMetadata, version)
	}
...
// encodeBinMetadata produces a version 2 $I file. Windows itself writes these
// files; this package only needs the encoder to test the decoder against a
// known-good layout.
func encodeBinMetadata(meta binMetadata) []byte {
